...
class RGBLampServer:
    ...
    def update_display(self, state):
        color = 'yellow' if state else 'black'
    ...
    def listen_for_commands(self):
        ...
        while self.running:
            try:
                data, addr = self.sock.recvfrom(1)  # Receive only 1 byte
                state = bool(data[0])
                print(f"\nReceived command from {addr}: State: {'ON' if state else 'OFF'}")

                # Update GUI in thread-safe way
                self.root.after(0, self.update_display, state)
